...
In Anlehnung an die Warenkorbeanalyse sollen die vergebene Themen für wissenschaftliche Publikationen analysiert werden,
...
können. Ein Datensatz zur Assoziationsanalyse besteht, also aus einer Menge von Transaktionen.
...
Die ist relative Häufigkeit der Transaktionen in denen X vorkommt zusammen mit Y.
...
\end{mdef}
\subsubsection{Lift}
Der Lift wird für die Stärke einer Regeli bzw. Bedeutung herrangezogen. 
\begin{mdef}[Lift]
\[
...
Der apriori-Algorithmus wurden 1994 von \cite{Agrawal:1994:FAM:645920.672836}  veröffentlicht, der sich fundamental zu denbis dato existierenden Algorithmen zum Finden von Assoziationsregeln unterscheidet. \\
...
Sei $L_k$ die Menge aller Mengen mit k Items, die mindestens eine Support von $\mathrm{minsupp}$ und $C_k$ die Menge möglicher Kandidaten für $L_k$. Im ersten Schritt wird die Häufigkeit aller Items in den Transaktionen aus $D$ berechnet und die Items, die mindestens $\mathrm{minsupp}$ haben bilden $L_1$. In der k-ten Iteration werden neue Kandidaten für $L-k$ erzeugt und wenn diese wieder $\mathrm{minsupp}$ übersteigen in $L_k$ aufgenommen. Der Algorithmus terminiert, wenn keiner der erzeugten Kandidatenmengen in $L_k$ aufgenommen wurde, also $L_k = \emptyset$.
\begin{algorithm}
...
\begin{algorithmic}
	...
	\FOR{$k = 2,\;L_{k-1} = \emptyset,\;k++$} 
		\STATE $L_k \gets \text{apriori-gen}(L_{k-1})$ \COMMENT{Erzeugung von k-elementigen Kanditatenmengen}
		\FORALL{$t \in D$} 
...
Die Erzeugung von $k$-elementigen Kandidatenmengen verläuft in zwei Schritten. Zunächst werden alle $(k-1)$-elementigen Itemmengen paarweise vereinigt, die eine gemeinsame $(k-2)$-elementige Teilmenge haben. Bei den so entstandenen $k$-elementigen Kanditaten wird Überprüft, ob alle $(k-1)$-elementigen Teilmengen in $L_k-1$ enthalten sind, wenn nicht, dann entferne die jeweilige Menge aus $C_k$.

\begin{algorithm}
	\caption{Erzeugung der Kanditatenmengen}
	\label{apriori-gen}
\begin{algorithmic}
	\STATE \COMMENT{Erzeugung von k-elementigen Kanditatenmengen}
	...
	\ENDIF
	\STATE \COMMENT{Entfernen von Kanditaten, deren (k-1)-elementigen Teilmengen der Obermenge bestehen}
	\FORALL{$c \in C_k$} 
